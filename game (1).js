...
// Games Played Counter - Server-based with localStorage fallback
async function initGamesCounter() {
    try {
        const response = await fetch('counter.php');
        if (!response.ok) throw new Error('Server not available');
        const data = await response.json();
        console.log('Initializing games counter (server):', data.count);
        updateCounterDisplay(data.count);
    } catch (error) {
        console.warn('Server counter not available, using localStorage fallback:', error);
...
async function incrementGamesPlayed() {
    try {
        ...
        if (!response.ok) throw new Error('Server not available');
        const data = await response.json();
        console.log('Incrementing games played to (server):', data.count);
        updateCounterDisplay(data.count);
    } catch (error) {
        console.warn('Server counter not available, using localStorage fallback:', error);
        // Fallback to localStorage
